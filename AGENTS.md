...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v7：工单侧与工单日报对齐 + 群维系只由海外侧汇总计一次 + 海外侧 10 项）**：`CAREER_SIDES = [工单, 海外]`；`CAREER_ITEMS = CAREER_TICKET_ITEMS.concat(CAREER_OVERSEAS_ITEMS)` = **26 项**（工单侧 16 / 海外侧 10）。
    - **工单侧 16 项 = 「工单日报」的全部数值字段（29）− 13**，**由 `DAILY_TICKET_FIELDS` 自动生成**（`CAREER_TICKET_ITEMS`，key 前缀 `t_`，label 直接用日报的 label）：
      - ⚠️ **以后往 `DAILY_TICKET_FIELDS` 加字段，工单侧会自动跟着有，不用手改**（verify 有断言锁这个构造方式）。用户的要求是「和工单日报对齐（除姓名和班次）」。
      - 排除规则 `CAREER_TICKET_EXCLUDE_PREFIX = ['g1_', 'g2_', 'g5_', 'g7_']`（用户逐轮定的，**别顺手改**）：
        - `g7_*`（⑦异世界勇者）→ 算海外业务，归海外侧的「异世界群维系」。
        - `g1_*`/`g2_*`/`g5_*`（①繁花/②乐缤纷/⑤梦幻）→ **已由海外侧「（梦幻/繁花/乐缤纷）群维系」汇总计入，工单侧再列一遍就是重复计量**（用户选了「去掉工单侧①②⑤」这个方案）。
        - **③④⑥（不差钱/你的江湖/伙伴弹途）不在那个汇总里，保留在工单侧**。
  ...
  - **两套口径**：`累计工作量` = 26 项（界面两侧卡片与之对齐）；`登记表 14 列口径` = 登记表那 14 列（10 列已归入两侧统计，4 列只存档）。**用上传的表导入时两者都是 5623，与原表完全一致**（因为表里那 4 个只存档列全是 0）。顶部写明「工单侧 X + 海外侧 Y = 累计 Z；其中登记表 14 列口径 W」。
    - 群维系不再重复计量：①~⑥ 逐平台里，**①②⑤ 只在海外侧的（梦幻/繁花/乐缤纷）群维系里算一次**，③④⑥ 只在工单侧算一次。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、工单日报字段自动进统计、①②⑤ 不在工单侧、海外日报新增 3 字段参与统计、别名、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **105**；`tools/verify.mjs` 加了一整块客服生涯断言（工单侧由 DAILY_TICKET_FIELDS 生成 + 排除前缀 g1_/g2_/g5_/g7_ + 工单侧 16 项 + 海外侧 10 项覆盖海外日报全部数值字段 + 14 列列名对得上（含别名）+ 只存档 4 项 + 持久化 key 等）。
