...
| 命令 | 内容 |
|---|---|
| `npm run verify` | 静态结构校验 + `node --check` 语法 + jsdom 集成冒烟 101 项 + 「接口挂起时界面仍可用」+ 服务端冒烟 12 项 |
...
`npm run verify` 失败就不要提交。它一共 5 段：静态结构 → 语法 → jsdom 集成冒烟 **101 项** → 接口挂起时界面仍可用 → 服务端冒烟 **12 项**。
...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v2，按用户要求改成两侧分开）**：`CAREER_SIDES = [工单, 海外]` 两侧；`CAREER_ITEMS` 共 **23 项**，每项带 `side`（ticket/overseas）+ `kind`（类别标签）+ `from`（日报字段映射）。**工单侧 16 项 / 海外侧 7 项**，跟日报的两个分栏一一对应。
    - 工单侧（取「工单日报」）：异世界群维系←`g7_*`、（繁花+乐缤纷+梦幻）群维系←`g1_*+g2_*+g5_*`、③不差钱群维系←`g3_*`、④你的江湖群维系←`g4_*`、⑥伙伴弹途群维系←`g6_*`、SSO国内工单←`ticket.sso`、53客服在线←`kf53`、支付宝在线←`alipay`、爱江山/繁花/乐缤纷/不差钱 CP后台、猫之城ios&B站评论回复\*、国内动物领主VIP\*、邮件+SDK（猫旅馆物语）\*、塔防FB\*
    - 海外侧（取「海外日报」）：海外SSO工单量←`overseas.sdk`、商店回复←`ios+google`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`、海外CP后台工单\*
    - \* = 日报里没有对应字段，标 **「需补录」**，只能手工填。**没有去改日报正文格式**（那会改变你现有的日报输出）。
  - **`extra` 概念已删除**：原先挂在 extra、不计入总数的那些字段（53客服在线、支付宝在线、4 个 CP 后台、③④⑥ 群维系）**已全部升格为正式统计项**。`verify.mjs` 有一条不变式断言：**日报 36 个数值字段必须全部被某个统计项的 `from` 覆盖**（防止以后加日报字段时漏统计）。
  - **对外登记表口径单列**：`CAREER_SHEET_ITEMS`（14 项）单独成表，用于**粘贴导入**（按这些列名对齐）与跟外部《工作量登记表》**对账**。界面顶部会写清「工单侧 X + 海外侧 Y = 累计；其中登记表 14 项口径 Z（差额来自日报有、登记表未列的项）」。
    - ⚠️ 两边口径**故意不一致**：`累计工作量` = 23 项（日报全字段，更全）；`登记表口径` = 14 项（对外报表面）。导入的表数据两者相等；日常归档的天数会让 23 项口径更大。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **101**；`tools/verify.mjs` 加 18 条静态断言（23 项/两侧项数/14 项列名与登记表一致/36 个日报字段全覆盖/持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**39/39 全对**——79 完整记录天、累计 5623、**工单侧 171 + 海外侧 5452 = 5623**、6/7/8/9 月分别 19/1312、23/1614、21/1572、16/1125、最高单日 191@2026-08-05、日均 71.2、14 项累计全部一致。回放脚本是一次性的（`tools/out/`，未入库）。
