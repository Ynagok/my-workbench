...
- 本轮：新增子标签页 **「游戏活动」**（`data-tab="activity"`，与「话术生成/集卡分析」同级，切换逻辑无需改 JS），内含 **4 张攻略速查表**，数据来自小红书笔记《【全活动数据汇总】蒲公英/绣球/金币赛等》的 P2–P5 图：
  - 🌼 蒲公英弹弹乐（10 关：120/150/190/230/250/300/350/400/450/500 与名额）、🏵️ 绣球争芳宴（8 关 + 3 阶段奖励）、🏯 茶楼大赛/盛世大富商（12 关 + 3 阶段奖励）、🌸 春生雅艺会（5 组）。
  - 每张表都带**「累计」行**（本表求和，非原帖内容）：蒲公英 **2940**、绣球 **2000**、茶楼金币 **7000**。
  - **奖励列是图标识读**（原帖用游戏道具图标），界面上已放显式免责说明；数量列与累计是确定的表格数值。
  - 实现方式：用一次性字符串匹配补丁脚本插入（EOL 自适应，并断言**字节增量必须等于插入量**以防误改），`public/index.html` 纯新增 118 行、**0 删除**。`tools/smoke.cjs` 加 **8b** 节 11 条（标签存在/点击激活/四表数据与累计逐行校验），`tools/verify.mjs` 加 2 条静态断言；冒烟 44 → **55**。
...
3. **「游戏活动」里的奖励列条目名待核对**：原帖奖励是游戏道具**图标**，当前表里的 `⚡×15`、`🎁×1`、`🧰×1`、`🪙×1`、`🍃×1`、`✂️×1`、`🍕×1`、`🚩×1` 等是**识图转写**，图标相近（红包/礼物盒/宝箱/福袋）容易认错。**数量列与累计行是确定的**。要改成准确道具名，需要用户给出对照（例如「⚡ 就是体力」），改完记得同步 `tools/smoke.cjs` 8b 节的断言。
