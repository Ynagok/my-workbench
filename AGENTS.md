...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v5：两侧分开 + 海外日报新增 3 个字段 + 群维系只留两项）**：`CAREER_SIDES = [工单, 海外]`；**计入统计的 `CAREER_ITEMS` = 17 项**（工单侧 7 / 海外侧 10），每项带 `side` + `kind` + `from`。
    - 工单侧（取「工单日报」）：SSO国内工单←`ticket.sso`、53客服在线←`kf53`、支付宝在线←`ticket.alipay`、爱江山/繁花/乐缤纷/不差钱 CP后台←`ticket.*_cp`。
    - 海外侧：**异世界群维系←`ticket.g7_*`（业务上算海外，但数据填在工单日报的⑦）**、**（梦幻/繁花/乐缤纷）群维系←`overseas.mhl_group`**、**海外CP后台工单←`overseas.cp_ticket`**、**国内工单←`overseas.cn_ticket`**、海外SSO工单量←`overseas.sdk`、商店回复←`overseas.ios+google`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`。
      - ⚠️ **「国内工单」与工单侧的「SSO国内工单」是两个不同的项**（用户确认）：前者在海外日报填，后者来自工单日报的 SSO工单。
      - ⚠️ 海外侧那个群维系项，**登记表里写作「（繁花+乐缤纷+梦幻）群维系」**（同一个东西、顺序不同）——见下面的 `CAREER_IMPORT_ALIASES`。
  ...
  - **`CAREER_IMPORT_ALIASES`**：登记表列名跟本系统标签写法不同的，导入时按别名对齐。目前只有一条：`（繁花+乐缤纷+梦幻）群维系` → `mhl_group`（海外侧群维系项），**所以历史数据的 142 也会进海外侧统计**。
  - **①②③④⑤⑥群维系（`g1_*`~`g6_*`）共 19 个日报字段完全不采集**（既不在统计层也不在存档层）。据用户口径：群维系只分「异世界群维系」和「（梦幻/繁花/乐缤纷）群维系」两项，都算海外业务。
  - **`extra` 概念早已删除**：现在只有「计入统计（17）」与「只存档（4）」两层。
  - **两套口径**：`累计工作量` = 17 项（界面两侧卡片与之对齐）；`登记表 14 列口径` = 登记表那 14 列（其中 10 列已归入两侧统计，4 列只存档）。**用上传的表导入时两者都是 5623，与原表完全一致**（因为表里那 4 个只存档列全是 0）。顶部写明「工单侧 X + 海外侧 Y = 累计 Z；其中登记表 14 列口径 W」。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、海外日报新增 3 字段参与统计、群维系列名别名、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **105**；`tools/verify.mjs` 现有静态断言数十条（17 项计入统计 / 两侧 7+10 / 海外日报 3 新字段与正文 3 行 / 14 项列名与登记表一致（含别名）/ 只存档 4 项 / ①~⑥群维系 19 字段完全不采集 / 持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**38/38 全对，且现在所有指标都跟原表一致**——79 完整记录天；**工单侧 22 + 海外侧 5,601 = 累计 5,623（= 原表累计工作量）**；其中海外侧含「（梦幻/繁花/乐缤纷）群维系 142」（从登记表那列经别名导入）；**日均 71.2、最高单日 191 @ 2026-08-05**、月度 **1,312 / 1,614 / 1,572 / 1,125** 全部与原表看板一致；14 列逐项累计吻合；只存档的 4 列都是 0。回放脚本是一次性的（`tools/out/`，未入库）。
