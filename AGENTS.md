...
| 命令 | 内容 |
|---|---|
| `npm run verify` | 静态结构校验 + `node --check` 语法 + jsdom 集成冒烟 102 项 + 「接口挂起时界面仍可用」+ 服务端冒烟 12 项 |
...
`npm run verify` 失败就不要提交。它一共 5 段：静态结构 → 语法 → jsdom 集成冒烟 **102 项** → 接口挂起时界面仍可用 → 服务端冒烟 **12 项**。
...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v3：两侧分开 + 只留用户要的项）**：`CAREER_SIDES = [工单, 海外]`；**计入统计的 `CAREER_ITEMS` = 15 项**（工单侧 7 / 海外侧 8），每项带 `side` + `kind` + `from`。
    - 工单侧（取「工单日报」）：SSO国内工单←`ticket.sso`、53客服在线←`kf53`、支付宝在线←`ticket.alipay`、爱江山/繁花/乐缤纷/不差钱 CP后台。
    - 海外侧：**异世界群维系←`ticket.g7_*`（业务上算海外，但数据仍填在工单日报的⑦）**、海外SSO工单量←`overseas.sdk`、商店回复←`ios+google`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`、海外CP后台工单（无日报来源，需补录）。
  - **`CAREER_HIDDEN_ITEMS`（只存档、不计入统计）= 5 项**，按用户要求撤出统计：①②⑤群维系（`g1_*+g2_*+g5_*`）、猫之城ios&B站评论回复、国内动物领主VIP、邮件+SDK（猫旅馆物语）、塔防FB。
    - ⚠️ **它们仍然照常采集（有日报来源的照常折算）与导入**，只是不进两侧卡片、不进「累计工作量」，**只用于「对外登记表口径」那张对账表**。原因：这 5 项都是《工作量登记表》的列（①②⑤群维系累计 142），真删掉导入就跟原表对不上账（5623 会变 5481）。
    - 想彻底不认这 5 列，只要把 `CAREER_HIDDEN_ITEMS` 和 `CAREER_SHEET_ITEMS` 里的对应项一起删掉即可（同时要改 verify 的断言）。
  - **③④⑥群维系（`g3_*`/`g4_*`/`g6_*`）已彻底移除**：它们既不在计入统计的 15 项里，也不在只存档的 5 项里（不是登记表的列），所以日报里这 9 个字段**完全不采集**。
  - **`extra` 概念早已删除**：现在只有「计入统计（15）」与「只存档（5）」两层。
  - **两套口径并存，故意不一致**：`累计工作量` = 15 项（界面两侧卡片与之对齐）；`登记表 14 列口径` = 15 项 + 5 个存档列（跟外部表对账用）。顶部会写明「工单侧 X + 海外侧 Y = 累计 Z；其中登记表 14 列口径 W」。用上传的表导入时：**5481（统计口径） + 142（只存档的群维系） = 5623（原表合计）**。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、只存档项仍采集、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **102**；`tools/verify.mjs` 加 20 条静态断言（15 项计入统计 / 两侧 7+8 / 异世界归海外 / 14 项列名与登记表一致 / 撤出统计的 5 项 / ③④⑥已移除 / 未覆盖的字段恰好是那 19 个群维系字段 / 持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**36/36 全对**——79 完整记录天；**工单侧 22 + 海外侧 5,459 = 累计 5,481**；只存档的①②⑤群维系 142；**5,481 + 142 = 5,623（与原表合计一致）**；月度 1,310/1,567/1,540/1,064（原表对应月减去只存档的 2/47/32/61）；14 列逐项累计全部吻合。回放脚本是一次性的（`tools/out/`，未入库）。
