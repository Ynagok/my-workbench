...
| 命令 | 内容 |
|---|---|
| `npm run verify` | 静态结构校验 + `node --check` 语法 + jsdom 集成冒烟 59 项 + 「接口挂起时界面仍可用」 |
...
- 本轮：按方案1 补齐 `index.html` 缺失的来源映射 **253–257**（名称取自 GM 玩家页的 `opFrom` 列表）：
  ...
  - 方案1 的另一半「`"81": "体力排行榜"` 之后插活动缺项」**未做**——那部分叫法待定；另注：方案1 里「静态 45 → 47」与实际不符（`verify.mjs` 的 `ok()` 当时只有 30 处），别照抄那个数。
