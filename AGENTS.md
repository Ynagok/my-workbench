...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v4：两侧分开 + 海外日报新增 3 个字段）**：`CAREER_SIDES = [工单, 海外]`；**计入统计的 `CAREER_ITEMS` = 17 项**（工单侧 7 / 海外侧 10），每项带 `side` + `kind` + `from`。
    - 工单侧（取「工单日报」）：SSO国内工单←`ticket.sso`、53客服在线←`kf53`、支付宝在线←`ticket.alipay`、爱江山/繁花/乐缤纷/不差钱 CP后台←`ticket.*_cp`。
    - 海外侧：**异世界群维系←`ticket.g7_*`（业务上算海外，但数据填在工单日报的⑦）**、**（爱江山/繁花/乐缤纷）群维系←`overseas.group_ajs`**、**海外CP后台工单←`overseas.cp_ticket`**、**国内工单←`overseas.cn_ticket`**、海外SSO工单量←`overseas.sdk`、商店回复←`overseas.ios+google`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`。
      - ⚠️ **「国内工单」与工单侧的「SSO国内工单」是两个不同的项**（用户确认）：前者在海外日报填，后者来自工单日报的 SSO工单。
  - **日报改动（用户明确要求）**：`DAILY_OVERSEAS_FIELDS` 新增 3 个 number 字段 `cp_ticket`（海外CP后台工单）/ `cn_ticket`（国内工单）/ `group_ajs`（（爱江山/繁花/乐缤纷）群维系），`DAILY_DEFAULT_DATA.overseas` 同步加默认值 0，`generateDailyReport()` 的海外分支在「禁言/封号」那行后**新增 3 行正文**。所以**海外日报的正文输出变了**（多 3 行），海外表单从 8 个字段变 11 个。
  - **`CAREER_HIDDEN_ITEMS`（只存档、不计入统计）= 5 项**：《工作量登记表》里的列——（繁花+乐缤纷+梦幻）群维系←`ticket.g1_*+g2_*`、猫之城ios&B站评论回复、国内动物领主VIP、邮件+SDK（猫旅馆物语）、塔防FB。
    - ⚠️ **它们仍然照常采集（有日报来源的照常折算）与导入**，只是不进两侧卡片、不进「累计工作量」，**只用于「对外登记表口径」那张对账表**。原因：这些是登记表的列（①②群维系累计 142），真删掉导入就跟原表对不上账。
    - 那列名字里虽然有「梦幻」，但⑤梦幻字段已按用户要求不再采集——只有**导入**时才会如实带进那列的数（导入对口是精确的）。
  - **③④⑤⑥群维系（`g3_*`/`g4_*`/`g5_*`/`g6_*`）共 12 个字段完全不采集**（既不在统计层也不在存档层）。据用户口径：群维系只分「异世界群维系」和「（爱江山/繁花/乐缤纷）群维系」两项。
  - **`extra` 概念早已删除**：现在只有「计入统计（17）」与「只存档（5）」两层。
  - **两套口径并存，故意不一致**：`累计工作量` = 17 项（界面两侧卡片与之对齐）；`登记表 14 列口径` = 其中属于登记表列的那些 + 5 个存档列（跟外部表对账用）。顶部会写明「工单侧 X + 海外侧 Y = 累计 Z；其中登记表 14 列口径 W」。用上传的表导入时：**5481（统计口径） + 142（只存档的群维系） = 5623（原表合计）**。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、海外日报新增 3 字段参与统计、只存档项仍采集、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **105**；`tools/verify.mjs` 现有 **53 条**静态断言（17 项计入统计 / 两侧 7+10 / 海外日报 3 新字段与正文 3 行 / 14 项列名与登记表一致 / 撤出统计的 5 项 / ③④⑤⑥ 12 字段完全不采集 / 持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**38/38 全对**——79 完整记录天；**工单侧 22 + 海外侧 5,459 = 累计 5,481**；只存档的①②群维系 142（来自表里那列）；**5,481 + 142 = 5,623（与原表合计一致）**；月度 1,310/1,567/1,540/1,064（原表对应月减去只存档的 2/47/32/61）；14 列逐项累计全部吻合；海外CP后台工单已标「日报自动」。回放脚本是一次性的（`tools/out/`，未入库）。
