...
| 命令 | 内容 |
|---|---|
| `npm run verify` | 静态结构校验 + `node --check` 语法 + jsdom 集成冒烟 106 项 + 「接口挂起时界面仍可用」+ 服务端冒烟 12 项 |
...
`npm run verify` 失败就不要提交。它一共 5 段：静态结构 → 语法 → jsdom 集成冒烟 **106 项** → 接口挂起时界面仍可用 → 服务端冒烟 **12 项**。
...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型（v8：工单侧 = 工单日报全部字段；异世界只在工单侧计一次）**：`CAREER_SIDES = [工单, 海外]`；`CAREER_ITEMS = CAREER_TICKET_ITEMS.concat(CAREER_OVERSEAS_ITEMS)` = **38 项**（工单侧 29 / 海外侧 9）。
    ...
    - 海外侧 9 项（手写 `CAREER_OVERSEAS_ITEMS`）：**（梦幻/繁花/乐缤纷）群维系←`overseas.mhl_group`**（海外日报手填的汇总数）、**海外CP后台工单←`overseas.cp_ticket`**、**国内工单←`overseas.cn_ticket`**、海外SSO工单量←`overseas.sdk`、商店回复←`overseas.ios+google`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`。
      ...
      - ⚠️ **海外侧没有「异世界群维系」项了**：⑦的量已按用户清单计入工单侧，两边都留就是同一份 `g7_*` 算两次。
  ...
  - **`CAREER_HIDDEN_ITEMS`（只存档、不计入统计）= 5 项**：**异世界群维系**（聚合，来源同样是 `ticket.g7_*`，只为登记表那列对账——统计侧已按平台记在工单侧）、猫之城ios&B站评论回复、国内动物领主VIP、邮件+SDK（猫旅馆物语）、塔防FB。
    - ⚠️ 它们可导入/折算，但**不进两侧卡片、不进「累计工作量」，只进「对外登记表口径」对账表**——否则导入的表跟原表对不上账。
    - ⚠️ 所以导入那张表后会出现：**累计 5,616**（统计）**+ 7**（只存档的异世界）**= 5,623**（原表合计）。顶部那行会写明这个差额。这 7 的差是用户选「⑦ 归工单侧」造成的必然结果（登记表那列是聚合，统计侧是逐平台）。
  ...
  - **两套口径**：`累计工作量` = 38 项（界面两侧卡片与之对齐）；`登记表 14 列口径` = 登记表那 14 列（9 列已归入两侧统计，5 列只存档）。顶部写明「工单侧 X + 海外侧 Y = 累计 Z；其中登记表 14 列口径 W」。
    - ⚠️ 注意 ①②⑤ 与 ⑦ 现在**逐平台算在工单侧**；海外侧那个「（梦幻/繁花/乐缤纷）群维系」是你在海外日报里**另填的一个汇总数**（跟 ①②⑤ 不是同一份输入）。所以别把同一批群维系同时在两处各填一遍，否则总量会重复。
  ...
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算、两侧拆分、工单日报全部字段进工单侧、只存档项仍采集但不计入、海外日报新增 3 字段参与统计、别名、导入、补录、删除、导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **106**；`tools/verify.mjs` 加了一整块客服生涯断言（工单侧由 DAILY_TICKET_FIELDS 生成且不排除字段 + 工单侧 29 项 + 海外侧 9 项覆盖海外日报全部数值字段 + 海外侧无重复的异世界项 + 14 列列名对得上（含别名）+ 只存档 5 项 + 持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**39/39 全对**——79 完整记录天；**工单侧 22 + 海外侧 5,594 = 累计 5,616**，**+ 只存档的异世界群维系 7 = 5,623（= 原表累计工作量）**；海外侧含「（梦幻/繁花/乐缤纷）群维系 142」（登记表那列经别名导入）；最高单日 **191 @ 2026-08-05**；月度 1,312 / 1,614 / 1,572 / **1,118**（9 月少了只存档的异世界 7）；日均 71.1（原表含异世界的口径是 71.2）；14 列逐项累计全部吻合。回放脚本是一次性的（`tools/out/`，未入库）。
