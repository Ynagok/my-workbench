...
| 命令 | 内容 |
|---|---|
| `npm run verify` | 静态结构校验 + `node --check` 语法 + jsdom 集成冒烟 94 项 + 「接口挂起时界面仍可用」+ 服务端冒烟 12 项 |
...
`npm run verify` 失败就不要提交。它一共 5 段：静态结构 → 语法 → jsdom 集成冒烟 **94 项** → 接口挂起时界面仍可用 → 服务端冒烟 **12 项**。
...
- 本轮：新增 **「客服生涯」标签页**（`data-tab="career"`）——按外部《工作量登记表》的 **14 个工作类型列**统计工作量，日报的「工单」「海外」两栏自动折算入账。
  - **口径模型**：`CAREER_ITEMS`（14 项，含 `group` 三大类与 `from` 日报字段映射）、`CAREER_GROUPS = ['工单/后台','回复/社区','监控']`、`CAREER_KEY = 'careerData'`。
  - **能自动折算的 9 项**：异世界群维系←`g7_*`、群维系(繁花+乐缤纷+梦幻)←`g1_*+g2_*+g5_*`、海外SSO工单←`overseas.sdk`、海外邮件←`overseas.email`、海外FB←`overseas.fb`、商店回复←`overseas.ios+google`、SSO国内工单←`ticket.sso`、监控禁言←`overseas.mute`、监控封号←`overseas.ban`。
  - **日报里没有对应项的 5 项**（猫之城ios&B站评论回复 / 国内动物领主VIP / 邮件+SDK（猫旅馆物语）/ 塔防FB / 海外CP后台工单）：表里标 **「需补录」**，只能手工填。**没有去改日报正文格式**（那会改变你现有的日报输出）。
  - **日报里有、登记表未列的项**（53客服在线、支付宝在线、4 个 CP 后台、`g3_*`/`g4_*`/`g6_*` 群维系）自动进 `extra`，单独累计展示、**不计入**「累计工作量」（保持与登记表口径一致）。
  - **归档时机**：点「生成日报」时顺手归档当天；跨天自动刷新那条路径**故意不归档**（否则会把昨天的数记到今天）。同一天重复归档 = 覆盖。也可用页面上的「归档今日日报」按钮手动触发。
  - **补录/修正**：可指定任意日期载入→改数字→保存（`src:'manual'`）；也能删除某天。
  ...
  - **统计项**（用户要"更能量化工作成果"，已去掉面试/简历相关表述）：累计工作量、完整记录天数、日均、中位数、最高单日+日期、最低单日、标准差+变异系数、最长连续记录+当前连续、记录区间+断档天数、近 7/30 记录日日均、环比上月、月度汇总（天数/总量/日均/最高单日/环比）、14 项累计+占比+结构条、三大类结构占比、渠道覆盖广度（几项有产出 + 单日最多触达）、数据质量（待确认天数+日期明细、特殊问题清单）、CSV/JSON 导出。
  - **持久化**：走 `/api/data/careerData`（= 服务端 `data.json`，仓库在 G 盘所以是非 C 盘，多设备打开同一地址即共享；localStorage 兜底）。`DATA_FILE` 环境变量可把数据文件指定到别的盘。
  - **测试**：`tools/smoke.cjs` 加 **8c** 节（自动归档折算/导入/补录/删除/导出，含「打乱表头顺序仍按列名落位」「重复导入不翻倍」「总计为空不计入」），冒烟 60 → **94**；`tools/verify.mjs` 加 10 条静态断言（14 项列名与登记表一致、映射已接、持久化 key 等）。
  - **✅ 真实数据对账**：把上传的登记表 81 行（80 天）原样喂进粘贴导入，与表格自己的「月度汇总」「个人指标看板」逐项比对，**32/32 全对**——79 完整记录天、累计 5623、6/7/8/9 月分别 19/1312、23/1614、21/1572、16/1125、最高单日 191@2026-08-05、日均 71.2、14 项累计全部一致。回放脚本是一次性的（`tools/out/`，未入库）。
